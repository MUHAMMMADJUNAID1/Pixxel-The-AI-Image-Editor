...
export function CropContent() {
    ...
    // Get the currentlt selected or min image
    ...
    // Cleanup when component unmouts
        useEffect(() => {
            return () => {
                if(!isCropMode) {
                    exitCropMode();
        ...
        // Initilizee crop mode
        const initializeCropMode = () => {
            if(!image || isCropMode) return;

            // First remove any existing crop rectnangles
            ...
            // Make image none-selectable to prevent default scaling
        ...
        const createCropRectangle  = (image) => {
            ...
            const cropRectangle = new Rect({
              ...
              cropRectangle.on("scaling",  (e) => {
                ...
                // Apply aspect ratio constraint if seclected
        ...
        const exitCropMode  = () => {
            ...
            // Remove all reactianles from canvas (since we only use them fro cropping)
            removeAllCropRectangles();

            // Clear crop rect Refrences
            setCropRect(null);
            ...
            console.log("Crop Mode cleanup mode complete");
            
        };

        // Apply aspect Ratio contraont to crop rectangle
        const applyAspectRetio = (ratio) => {
            setSelectedRatio(ratio);

            if(!cropRect || ratio === null) return;

            // Calculate new Diamentsion maintinaing aspect ratio
        ...
        //Apply crop tranformation using fabric js built in cropping 
        const applyCrop =  async() => {
            if(!selectedImage || !cropRect) return;
            try {
                // Get crop reactangle bounds
               ...
               const cropHeight = Math.min(
                ...
                // Convart to image coordinate sysyem (acconting for image scaling)'
    ...
    return (
        <div className="space-y-6">
                ...
                {
                    isCropMode && (
                        <div className="bg-cyan-500/10 border-cyan-500/20 rounded-lg p-3">
                            <p className="text-cyan-400 text-sm font-medium">
                                Crop Mode active
                            </p>

                            <p className="text-cyan-300/80 text-xs mt-1">
                            Adjust the blue reactangle to set crop area</p>
                        </div>
                    )
                }
          ...
          <div className="grid grid-cols-3 gap-2">
            {ASPECT_RATIOS.map((ratio) => {
              ...
              return (
                <button
                  key={ratio.label}
                  onClick={() => applyAspectRetio(ratio.value)}
