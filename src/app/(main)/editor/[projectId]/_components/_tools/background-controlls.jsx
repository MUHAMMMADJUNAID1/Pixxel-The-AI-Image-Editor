...
export function BackgroundControls ({project}) {
    ...
    // Get the min image object from cnavas
    const getMainImage  = () => {
      ...
      // Background Removeal using Imagekit
      ...
      try {
        ...
        /// Ceate ImageKit transformation URL For Background Removal
        const bgRemovalUrl = currentImageUrl.includes("ik.imagekit.io") ? `${currentImageUrl.split("?")[0]}?tr=e-bgremove` : currentImageUrl;

        // Creete new Image with background removed 
        const processedImage = await FabricImage.fromURL(bgRemovalUrl, {
        ...
        const currentProps = {
          ...
          // IMPORTANT : Update coordinats after replacing the image 
          processedImage.setCoords();

          // Set as Active Update Coordinates after replacing the image
          canvasEditor.setActiveObject(processedImage)
          canvasEditor.calcOffset();
          canvasEditor.requestRenderAll();

          console.log("Background Removed Succesfully");
          }
         catch (error) {
    ...
    const handleImageBackground = async (imageUrl,imageId) => {
        ...
        try {
            //Download and trigger UNsplashdownow endopitn (requieded by unsplash)
            if(UNSPLASH_ACCESS_KEY){
                ...
                .catch(() => {}) // Silent fail for dowunload traking
            }
  ...
  console.log("Background set:", {
    ...
    return (
        <div className='space-y-6 relative h-full'>
            {/* AI background Removal - Oustsid of tabs */}
            ...
            <div>
                <h3 className='text-sm font-medium text-white mb-2'>
                    AI Background Removal
                     ...
                     Automically remove the background from your image using AI </p>
                </div>
                ...
                </Button>
                {
                    !getMainImage() && (
                        <p className="text-xs text-amber-400">
                            Please add an Image to the canvas first to remov its background 
                        </p>
                    )
                }
             </div>
             ...
             <Tabs defaultValue="color" className={"w-full"}>
                ...
                <TabsContent value='color' className={"space-y-4 mt-6"}>
                    ...
                    <div className="space-y-4">
                        <HexColorPicker color={backgroundColor}
                    ...
                    {/* Empty State */}
                    {
                        !isSearching && unsplashImages?.length === 0  && searchQuery && (
                            ...
                            >
                                ...
                                No Images found for "{searchQuery}"</p>
                                <p className="text-white/50 text-xs">
                                Tray a diffrent search term</p>
                            </div>
                        )
                    }
                    {/* Initial state */}
                    {
                        !searchQuery && unsplashImages?.length === 0 && (
                            <div className="text-center py-8">
                                ...
                                />
                                <p className="text-white/70 text-sm">
                                Search  for background Images</p>
