...
import { ArrowUp, ArrowDown, ArrowLeft, Wand2 } from "lucide-react";
...
export function AIExtenderControls ({project}){
  ...
  const applyExtension = async () => {
    ...
    try {
      ...
      console.log("Current URL:", currentImageUrl); // Debug
      
      const extendedUrl = buildExtensionUrl(currentImageUrl);
      console.log("Extended URL:", extendedUrl); // Debug
      ...
      await new Promise((resolve) => {
        if (extendedImage.width && extendedImage.height) {
          resolve();
        } else {
          extendedImage.on('load', resolve);
        }
      });
  
      console.log("Extended image dimensions:", extendedImage.width, extendedImage.height); // Debug
      ...
      // Scale to fit canvas (but not smaller than original scale)
      ...
      // ✅ Don't leave canvas empty on error - keep the original image
      canvasEditor.requestRenderAll();
...
if (!canvasEditor) {
  ...
  return (
        ...
        >
            ...
            {/* Extension Amount */}
            <div>
                <div className="flex justify-baseline items-center mb-2">
                  <Label className={"text-xs text-white"}>Extension Amount</Label>
      ...
      <Button
       ...
       disabled={!selectedDirection}  // State variable
      className={"w-full"}
      variant={"primary"}
      >
        <Wand2
