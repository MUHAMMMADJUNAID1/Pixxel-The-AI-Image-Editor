...
export function AdjustControls() {
    ...
    const extractFilterValues = (imageObject) => {
        ...
        imageObject.filters.forEach((filter) => {
            ...
            if(config) {
                ...
                if(config.key === "hue"){
                    extractFilterValues[config.key] = Math.round(
    ...
    if(!canvasEditor) {
        return (
            <div className="p-4">
                ...
                Load an image start adjusting</p>

            </div>
        )
    }

    const acitveImage = getActiveImage();
    if(!acitveImage){
        return (
            <div className="p-4">
                <p className="text-white/70 text-sm">
                Select an image to adjust filters</p>
            </div>
        )
    }
    ...
    return (
        ...
        Adjustments are applied in real-life. Use the Reset button to restore Original Values</p>
        </div>
