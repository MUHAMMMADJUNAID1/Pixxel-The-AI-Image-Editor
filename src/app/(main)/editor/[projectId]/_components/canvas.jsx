...
function CanvasEditor({ project }) {
    ...
    const isInitializing = useRef(false); // ADDED: Prevent double initialization
    ...
    useEffect(() => {
        if (isMounted.current || !canvasRef.current || !project || canvasEditor) return;
           isMounted.current = true;
        const initializeCanvas = async () => {
            isInitializing.current = true; // ADDED: Set flag
            setIsLoading(true);

            // ADDED: Check if canvas element already has a fabric instance
            ...
            setIsLoading(false);
            isInitializing.current = false; // ADDED: Reset flag
        };
    ...
    }, [project, canvasEditor, setCanvasEditor]); // FIXED: Added proper dependencies
    ...
    }, [canvasEditor, project, updateProject]); // FIXED: Added dependencies
    ...
    useEffect(() => {
        ...
        canvasEditor.requestRenderAll(); // ADDED: Ensure cursor change is rendered
