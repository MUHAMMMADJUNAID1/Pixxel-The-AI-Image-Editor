...
export function EditorTopBar({project}){
    ...
    const handleUndo = async() => {
        ...
        try {
            //Move current state to redu stack
            const currentState = JSON.stringify(canvasEditor.toJSON());
            setRedoStack((prev) => [...prev, currentState]);

            // Remove last state from the undo stack and apply the previous one
    ...
    const handleRedo = async() => {
        ...
        try {
            // Get the lastest state from  redo stack
    ...
    const handleManualSave = async() => {
        ...
        try {
            const canvasJSON = canvasJSON.toJSON();
            await updateProject({
    ...
    const handleExport = async(exportConfig) => {
      ...
      try {
        //Reset current canvas state for restoration
            ...
            <div className="flex items-center gap-3">
                ...
                <DropdownMenu>
                  ...
                  className={"w-56 bg-slate-800 border-slate-700"}>
                    ...
                    >
                      Export Resultion : {project.width} x {project.height}px
                    </div>
